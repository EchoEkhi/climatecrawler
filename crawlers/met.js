...
async function met() {
    ...
    for (const observation of observations) timeDifferences.push(Math.abs(new Date(yesterday.getFullYear(), yesterday.getMonth(), yesterday.getDate(), 18, 0, 0) - new Date(observation.ReportStartDateTime)))

    // find the minimum value and let data be that record
    const data = observations[timeDifferences.indexOf(Math.min.apply(Math, timeDifferences))]
